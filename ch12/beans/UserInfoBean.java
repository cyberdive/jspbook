...
public class UserInfoBean implements java.io.Serializable {
  ...
  /**
   * This array holds a sorted list of ad ids, ranked in the order which of
   * which are likely to be of the most interest tp the user
   */
  private int adRankings[] = new int[0];

  /**
   * The last ad shown to the user.  This is maintained so the user will
   * never see the same ad twice in a row.
   */
   ...
   * In chapter 12, this also re-sorts the list of ads based on the
   * new interest information.
   */
  public void setArticleId(int articleId) {
    ...
    /* Recompute the ad rankings, using a least-squared approach.  This
       is done here instead of in the getAd() method, becauser this method
       is called after a user gets a page, but getAd() is called before 
       the page is sent to the user.  Thereofre, the user has to wait
       for getAd(), but not for this.

       This is by no means the most efficient way to compute this
       information, but it should serve to demonstrate the principle.
    */

    try {
    int adKeywords[][] = AdHolder.getKeywords();
    int distances[]    = new int[adKeywords.length];

    for(int i=0;i<adKeywords.length;i++) {
      distances[i] = 0;

      for(int j=0;j<adKeywords[i].length;j++) {
	Integer userInterest = (Integer) interests.get(new Integer(j));
	int interest = (userInterest == null) ? 0 : userInterest.intValue();
	int tmp      = adKeywords[i][j] - interest;
	
	distances[i] = distances[i] + tmp * tmp;
      }
    }

    /* Now sort the distances */
    adRankings = new int[adKeywords.length];

    for(int i=0;i<adRankings.length;i++) adRankings[i] = i;

    for(int i=0;i<adRankings.length;i++) {
      for(int j=0;j<adRankings.length;j++) {
	if(distances[adRankings[i]] > distances[adRankings[j]]) {
	  int tmp = adRankings[i];
	  adRankings[i] = adRankings[j];
	  adRankings[j] = tmp;
	}
      }
    }
    } catch (Throwable t) {
      System.err.println("Unable to recompute rankings: ");
  ...
  /**
   * Get the ad the user is most interested in.  If that happens to be
   * the same ad they were last shown, instead pull an ad from the top
   * 10 ads of interest at random.  Also notify the AdHolder bean
   * that the ad has been seen.
   */
  public String getAd() {
    try {
    int bestAd = 0;

    if(adRankings != null && adRankings.length > 0) {
      bestAd = adRankings[0];
    
      if(bestAd == lastAd) {
	int maxAd = (adRankings.length < 10) ? adRankings.length : 10;
	bestAd    = adRankings[(int) System.currentTimeMillis() % maxAd];
      }
    } else {
      bestAd = (int) System.currentTimeMillis() % 
	        AdHolder.getKeywords().length;
    }
    
    AdHolder.adShown(bestAd);    
    lastAd = bestAd;

    return AdHolder.getAdText(bestAd);
    } catch (Throwable t) {
      System.err.println("Unable to get ad");
      t.printStackTrace(System.err);
      return "Default ad";
    }
  }

  /* The follow methods support e-commerce */
  ...
  public void setShipAddr(String s)      {shipAddr = s;}
  public void setBillAddr(String s)      {billAddr = s;}
  public void setCreditCardNum(String s) {creditCardNum = s;}
  public void setExpMonth(String s)      {expMonth = s;}
  public void setExpYear(String s)       {expYear = s;}


  public boolean canOrder() {
    return shipAddr != null &&
      billAddr      != null &&
      creditCardNum != null &&
      expMonth      != null &&
      expYear       != null;
